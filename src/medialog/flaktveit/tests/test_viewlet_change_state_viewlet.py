# -*- coding: utf-8 -*-
from medialog.flaktveit.interfaces import IMedialogFlaktveitLayer
from medialog.flaktveit.testing import MEDIALOG_FLAKTVEIT_FUNCTIONAL_TESTING
from medialog.flaktveit.testing import MEDIALOG_FLAKTVEIT_INTEGRATION_TESTING
...
class ViewletIntegrationTest(unittest.TestCase):
    ...
    def test_change_state_viewlet_is_registered(self):
        view = BrowserView(self.portal['other-document'], self.request)
        manager_name = 'plone.abovecontenttitle'
        alsoProvides(self.request, IMedialogFlaktveitLayer)
        ...
        manager.update()
        my_viewlet = [v for v in manager.viewlets if v.__name__ == 'change-state-viewlet']  # NOQA: E501
        self.assertEqual(len(my_viewlet), 1)

    # XXX would be nice to have this test working:
    # def test_change_state_viewlet_is_not_available_on_newsitem(self):
...
class ViewletFunctionalTest(unittest.TestCase):
    ...
    layer = MEDIALOG_FLAKTVEIT_FUNCTIONAL_TESTING
